...
public class ZahlungsdatenDAO extends BaseDAO<Zahlungsdaten> {
    ...
    public int findOrCreateZahlungsdaten(String name, String iban, String bic)
        ...
        if (name.trim().isEmpty() || iban.trim().isEmpty() || bic.trim().isEmpty()
                || "TEST".equalsIgnoreCase(iban.trim())
                || "TEST".equalsIgnoreCase(bic.trim())) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (check != null && !check.getName().isEmpty() && !check.getIBAN().isEmpty() && !check.getBIC().isEmpty()
                        && !"TEST".equalsIgnoreCase(check.getIBAN())
